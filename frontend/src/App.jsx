        ...
        <div className="min-h-screen p-6 md:p-12 text-slate-100">
            <div className="max-w-6xl mx-auto space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="rounded-3xl p-6 bg-gradient-to-r from-indigo-500/10 via-purple-400/5 to-transparent border border-indigo-500/20">
                        <div className="flex items-start gap-4">
                            <div className="p-3 rounded-2xl bg-indigo-500/15 border border-indigo-500/30 flex-shrink-0">
                                <Mail className="w-6 h-6 text-indigo-400" />
                            </div>
                            <div className="flex-1 min-w-0">
                                <h3 className="text-lg font-semibold text-indigo-200 mb-1">邮件中心 · 已上线</h3>
                                <p className="text-sm text-indigo-200/70 leading-relaxed mb-3">
                                    月度经营报表自动发送与多场景告警邮件模板管理。内置月度报表、温度告警、失王告警、采蜜完成四套模板，
                                    支持HTML可视化编辑、实时预览与发送测试，每月1日凌晨自动群发报表给场主。
                                </p>
                                <div className="flex flex-wrap items-center gap-2">
                                    <button
                                        onClick={() => {
                                            const url = new URL(window.location.href);
                                            url.searchParams.set('screen', 'email-center');
                                            window.open(url.toString(), '_blank');
                                        }}
                                        className="px-3 py-1.5 rounded-lg bg-indigo-500/20 border border-indigo-500/40 text-indigo-300 text-xs font-medium hover:bg-indigo-500/30 transition-all flex items-center gap-1.5"
                                    >
                                        <Mail className="w-3.5 h-3.5" />
                                        进入邮件中心
                                    </button>
                                    <code className="px-2 py-1 rounded-md bg-slate-900/60 text-indigo-300/90 text-xs border border-indigo-500/20 font-mono">
                                        ?screen=email-center
                                    </code>
                                </div>
                            </div>
